...
router.post('/test', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!user?.pushToken) return res.status(400).json({ error: 'Push não configurado' });
    ...
    if (success) res.json({ message: 'Notificação enviada' });
    else res.status(500).json({ error: 'Falha ao enviar push' });
  } catch (error) {
