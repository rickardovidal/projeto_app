    ...
    cron.schedule('0 * * * *', async () => {
        ...
        try {
            ...
            for (const assignment of assignments) {
                if (!assignment.deadline || !assignment.notifyBefore)
                    continue;
                ...
                if (now >= notifyThreshold) {
                    ...
                    if (!pushSent && user.email) {
                        await sendEmailNotification(user.email, `Lembrete: ${assignment.title}`, `<p>Olá ${user.name || ''},</p><p>O teu trabalho <strong>${assignment.title}</strong> tem entrega marcada para breve.</p>`);
                    }
